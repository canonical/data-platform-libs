...
async def test_deploy_charms(ops_test: OpsTest, application_charm, database_charm):
    ...
    await asyncio.gather(
        ops_test.model.deploy(
            application_charm, application_name=APPLICATION_APP_NAME, num_units=2, series="focal"
        ),
        ops_test.model.deploy(
            database_charm,
            resources={
...
async def test_two_applications_doesnt_share_the_same_relation_data(
    ...
    await ops_test.model.deploy(
        application_charm, application_name=another_application_app_name, series="focal"
    )
    ...
    await ops_test.model.applications[another_application_app_name].remove_relation(
        f"{another_application_app_name}:{FIRST_DATABASE_RELATION_NAME}", f"{DATABASE_APP_NAME}"
    )
