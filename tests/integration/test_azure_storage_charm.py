...
AZURE_STORAGE_APP_NAME = "azure-storage-provider"
APP_NAMES = [APPLICATION_APP_NAME, AZURE_STORAGE_APP_NAME]
FIRST_S3_RELATION_NAME = "first-azure-storage-credentials"
SECOND_S3_RELATION_NAME = "second-azure-storage-credentials"


@pytest.mark.abort_on_fail
async def test_deploy_charms(
    ops_test: OpsTest, application_azure_storage_charm, azure_storage_charm, dp_libs_ubuntu_series
):
    """Deploy both charms (application and azure storage provider app) to use in the tests."""
    # Deploy both charms (2 units for each application to test that later they correctly
    # set data in the relation application databag using only the leader unit).
    await asyncio.gather(
        ops_test.model.deploy(
            application_azure_storage_charm,
            application_name=APPLICATION_APP_NAME,
            num_units=2,
            series=dp_libs_ubuntu_series,
        ),
        ops_test.model.deploy(azure_storage_charm, application_name=AZURE_STORAGE_APP_NAME, num_units=2, series="jammy"),
    )
    await ops_test.model.wait_for_idle(apps=[AZURE_STORAGE_APP_NAME], status="active", wait_for_exact_units=2)
...
async def test_s3_relation_with_charm_libraries(ops_test: OpsTest):
    ...
    await ops_test.model.add_relation(
        AZURE_STORAGE_APP_NAME, f"{APPLICATION_APP_NAME}:{FIRST_S3_RELATION_NAME}"
    )
...
async def test_two_applications_doesnt_share_the_same_relation_data(
    ...
    await ops_test.model.wait_for_idle(apps=[AZURE_STORAGE_APP_NAME, APPLICATION_APP_NAME], status="active")
    ...
    await ops_test.model.add_relation(
        f"{another_application_app_name}:{FIRST_S3_RELATION_NAME}", AZURE_STORAGE_APP_NAME
    )
...
async def test_an_application_can_request_multiple_s3_providers(ops_test: OpsTest):
    ...
    await ops_test.model.add_relation(
        f"{APPLICATION_APP_NAME}:{SECOND_S3_RELATION_NAME}", AZURE_STORAGE_APP_NAME
    )
    await ops_test.model.wait_for_idle(apps=APP_NAMES, status="active")
