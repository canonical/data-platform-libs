...
class DatabaseCharm(CharmBase):
    ...
    def __init__(self, *args):
        ...
        self.framework.observe(
            self.provider.on.database_role_permissions_changed,
            self._on_database_role_permissions_changed,
        )
    ...
    def _on_database_role_permissions_changed(self, _) -> None:
        pass
...
class KafkaCharm(CharmBase):
    ...
    def __init__(self, *args):
        ...
        self.framework.observe(
            self.provider.on.topic_role_permissions_changed,
            self._on_topic_role_permissions_changed,
        )
    ...
    def _on_topic_role_permissions_changed(self, _) -> None:
        pass
...
class OpenSearchCharm(CharmBase):
    ...
    def __init__(self, *args):
        ...
        self.framework.observe(
            self.provider.on.index_role_permissions_changed,
            self._on_index_role_permissions_changed,
        )
    ...
    def _on_index_role_permissions_changed(self, _) -> None:
        pass
...
class TestDatabaseProvides(DataProvidesBaseTests, unittest.TestCase):
    ...
    @patch.object(DatabaseCharm, "_on_database_role_permissions_changed")
    def test_on_role_permissions_changed(self, _on_database_role_permissions_changed):
        """Asserts that the correct hook is called when role permissions are changed."""
        # Simulate the request of a new user plus extra roles.
        self.harness.update_relation_data(
            self.rel_id,
            "application",
        ...
        # Assert the correct hook is called.
        _on_database_role_permissions_changed.assert_called_once()

        # Assert the database name and the role info are accessible in the providers charm library event.
        event = _on_database_role_permissions_changed.call_args[0][0]
        assert event.database == DATABASE
        assert event.role_type == ROLE_USER
        assert event.role_permissions == ROLE_PERMISSIONS
        assert event.extra_user_roles == EXTRA_USER_ROLES
...
class TestKafkaProvides(DataProvidesBaseTests, unittest.TestCase):
    ...
    @patch.object(KafkaCharm, "_on_topic_role_permissions_changed")
    def test_on_role_permissions_changed(self, _on_topic_role_permissions_changed):
        """Asserts that the correct hook is called when role permissions are changed."""
        # Simulate the request of a new user plus extra roles.
        self.harness.update_relation_data(
            self.rel_id,
        ...
        # Assert the correct hook is called.
        _on_topic_role_permissions_changed.assert_called_once()

        # Assert the topic name and role info are accessible in the providers charm library event.
        event = _on_topic_role_permissions_changed.call_args[0][0]
        assert event.topic == TOPIC
        assert event.role_type == ROLE_USER
        assert event.role_permissions == ROLE_PERMISSIONS
        assert event.extra_user_roles == EXTRA_USER_ROLES
...
class TestOpenSearchProvides(DataProvidesBaseTests, unittest.TestCase):
    ...
    @patch.object(OpenSearchCharm, "_on_index_role_permissions_changed")
    def test_on_role_permissions_changed(self, _on_index_role_permissions_changed):
        """Asserts that the correct hook is called when role permissions are changed."""
        # Simulate the request of a new user plus extra roles.
        self.harness.update_relation_data(
            self.rel_id,
        ...
        # Assert the correct hook is called.
        _on_index_role_permissions_changed.assert_called_once()

        # Assert the index name and the role info are accessible in the providers charm library event.
        event = _on_index_role_permissions_changed.call_args[0][0]
        assert event.index == INDEX
        assert event.role_type == ROLE_USER
        assert event.role_permissions == ROLE_PERMISSIONS
        assert event.extra_user_roles == EXTRA_USER_ROLES
...
class TestDatabaseRequires(DataRequirerBaseTests, unittest.TestCase):
    ...
    def test_on_database_role_created(self, _on_database_role_created):
        ...
        self.harness.update_relation_data(
            self.rel_id,
            self.provider,
            {"role-type": ROLE_USER, "username": "test-username", "password": "test-password"},
        )

        # Assert the correct hook is called.
        _on_database_role_created.assert_called_once()

        # Check that the role-type, username and password are present in the relation
    ...
    def test_on_database_role_created_secrets(self, _on_database_role_created):
        ...
        self.harness.update_relation_data(
            self.rel_id,
            self.provider,
            {"role-type": ROLE_USER, f"{PROV_SECRET_PREFIX}user": secret.id},
        )
