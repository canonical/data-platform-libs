...
class SparkServiceAccountProviderCharm(CharmBase):
    ...
    def __init__(self, *args):
        super().__init__(*args)
        self.provider = SparkServiceAccountProvider(
...
class SparkServiceAccountRequirerCharm(CharmBase):
    ...
    def _consume_service_account(self, service_account: str | None, spark_properties: str | None) -> None:
...
class TestSparkServiceAccountProvider:
    ...
    def context(self):
        return Context(
            charm_type=SparkServiceAccountProviderCharm, meta=SparkServiceAccountProviderCharm.META, actions=SparkServiceAccountProviderCharm.ACTIONS
        )
    ...
    def test_service_account_property_changed(self, ):
        relation = self.get_relation()
        state1 = State(
            relations=[relation],
            leader=True
        ...
        state2 = self.context.run(self.context.on.relation_changed(relation), state1)
        state3 = self.context.run(self.context.on.action("add-config", params={"conf": "newkey=newval"}), state2)
    ...
    def test_service_account_released(self, mock_delete_sa):
        ...
        state2 = self.context.run(self.context.on.relation_changed(relation), state1)
        state3 = self.context.run(self.context.on.relation_broken(relation), state2)
...
class TestSparkServiceAccountRequirer:
    ...
    def test_spark_properties_changed(self, mock_consume_sa):
        ...
        state2 = self.context.run(self.context.on.secret_changed(props_secret), state1)
    ...
    def test_service_account_gone(self, mock_consume_sa):
        ...
        state2 = self.context.run(self.context.on.relation_broken(relation), state1)
